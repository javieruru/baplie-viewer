...
function generateMOVINS(index, options) {
    ...
    const globalPod = (options.pod || '').trim().toUpperCase();
    const applyGlobal = !!options.applyGlobalPolPod;
    ...
    if (!carrier) warnings.push({ code: 'CARRIER_EMPTY', message: 'No se indicó carrier/operador. Se usará como NAD+CA de respaldo cuando falte el operador por contenedor.' });
    if (!globalPol) warnings.push({ code: 'GLOBAL_POL_EMPTY', message: 'No se indicó POL global de respaldo.' });
    if (!globalPod) warnings.push({ code: 'GLOBAL_POD_EMPTY', message: 'No se indicó POD global de respaldo.' });
    ...
    const header = [
        ...
        `TDT+20++++${carrier}:172:20++++:103:ZZZ:${vessel}:600`,
        `LOC+175+${globalPol}:139:6`,
        `LOC+61+${globalPod}:139:6`,
    ...
    containers.forEach((c, idx) => {
        ...
        let pol = applyGlobal ? globalPol : (c.pol || '').trim().toUpperCase();
        let pod = applyGlobal ? globalPod : (c.pod || '').trim().toUpperCase();
        if (movinsIsBadLocCode(pol)) {
            if (!applyGlobal) warnings.push({ code: 'POL_FALLBACK', message: `${label}: POL original inválido ("${c.pol || ''}"), se usó el POL global confirmado.`, container: label });
            pol = globalPol;
        }
        if (movinsIsBadLocCode(pod)) {
            if (!applyGlobal) warnings.push({ code: 'POD_FALLBACK', message: `${label}: POD original inválido ("${c.pod || ''}"), se usó el POD global confirmado.`, container: label });
            pod = globalPod;
        }
        ...
        const operator = (c.slotOperator || carrier || '').trim();
        if (!operator) {
    ...
    const allSegments = [
        `UNB+UNOA:1+${carrier}+UNKNOWN+${movinsFormatUNBDateTime(docDate)}+${interchangeRef}`,
        ...bodySegments,
        unt,
    ...
    return {
        ...
        warnings,
        errors,
        meta: { vessel, voyage, carrier, pol: globalPol, pod: globalPod, containerCount: containers.length, interchangeRef }
    };
}
...
function validateMOVINS(movinsText, context) {
    ...
    if (!has('TDT+')) push('error', 'MISSING_TDT', 'Falta el segmento TDT (buque/voyage/carrier).');
    if (!has('LOC+175')) push('error', 'MISSING_LOC175', 'Falta LOC+175 (POL de cabecera).');
    if (!has('LOC+61')) push('error', 'MISSING_LOC61', 'Falta LOC+61 (POD de cabecera).');
    ...
    if (context) {
        ...
        if (!context.pol) push('warning', 'CTX_POL_EMPTY', 'POL global no confirmado.');
        if (!context.pod) push('warning', 'CTX_POD_EMPTY', 'POD global no confirmado.');
    }
...
function openMOVINSModal(index) {
    ...
    document.getElementById('movinsVoyage').value = '';
    document.getElementById('movinsCarrier').value = '';
    document.getElementById('movinsPol').value = movinsGuessGlobalLocCode(baplie.containers || [], 'pol');
    document.getElementById('movinsPod').value = movinsGuessGlobalLocCode(baplie.containers || [], 'pod');
    document.getElementById('movinsApplyGlobalPolPod').checked = false;
...
function movinsReadOptionsFromModal() {
    ...
    return {
        ...
        carrier: document.getElementById('movinsCarrier').value,
        pol: document.getElementById('movinsPol').value,
        pod: document.getElementById('movinsPod').value,
        applyGlobalPolPod: document.getElementById('movinsApplyGlobalPolPod').checked,
